...
/*========================= Read input voltage and current sampling result =========================*/
	while(!(ADC->RDY & 0x1)) {;}
#if ISNS_HALL
	gsPFC_Drive.sIinCtrl.f16out = ADC->RSLT[8]; 
	gsPFC_Drive.sIinCtrl.f16IsnsRec = gsPFC_Drive.sFlag.Pol? gsPFC_Drive.sIinCtrl.f16out:(-gsPFC_Drive.sIinCtrl.f16out);
#endif

#if !SPLL_EN
	while(!(ADC->RDY & 0x2)) {;}
	gsPFC_Drive.sVacTest.f16Vac_L = ADC->RSLT[1];
	gsPFC_Drive.sVacTest.f16Vac_N = ADC->RSLT[9];
	gsPFC_Drive.sVacTest.f16Vac = MLIB_Sub_F16(gsPFC_Drive.sVacTest.f16Vac_L,gsPFC_Drive.sVacTest.f16Vac_N);
	gsPFC_Drive.sVacTest.f16Vac = GDFLIB_FilterIIR1_F16(gsPFC_Drive.sVacTest.f16Vac,&gsPFC_Drive.sVacTest.sFilterParam);
	/* differential measure is used in other platform, but single ended measure is used in this platform,
	 * in order to keep the same control coefficient, divide the sampling result by 2 */
	gsPFC_Drive.sVacTest.f16Vac = MLIB_Mul_F16(gsPFC_Drive.sVacTest.f16Vac, FRAC16(0.5)); 
...
{	
	...
	gsPFC_Drive.sVoutCtrl.f16out = ADC->RSLT[0];
