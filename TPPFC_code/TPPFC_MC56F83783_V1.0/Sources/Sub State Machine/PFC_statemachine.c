...
static void PFC_StateFault(void)
{
	...
	if(gu16LLCRun == 1 && (QSCI0->STAT & QSCI_STAT_TDRE_MASK))
	{
		QSCI0->DATA = 0x01;
		QSCI0->DATA = 0x33; /* 0x55, start LLC; 0x33, stop LLC; else no effect */
		QSCI0->DATA = 0x32; /* check sum */
	}
#endif
...
static void PFC_StateRun(void)
{
	...
	if(gu16LLCVprimOK == 0 && gsPFC_Runsub != SOFTSTART)
	{
		if(QSCI0->STAT & QSCI_STAT_TDRE_MASK)
		{
			...
			QSCI0->DATA = ucLLC_VprimH;
			QSCI0->DATA = ucLLC_VprimL;
			QSCI0->DATA = 0x02 ^ ucLLC_VprimH ^ ucLLC_VprimL; /* check sum */
		}
	}
	...
	else if(gu16LLCVprimOK == 1)
	{
		...
		{
			QSCI0->DATA = 0x01;
			QSCI0->DATA = 0x55; /* 0x55, start LLC; 0x33, stop LLC; else no effect */
			QSCI0->DATA = 0x01 ^ 0x55;
		}
	}
#endif
