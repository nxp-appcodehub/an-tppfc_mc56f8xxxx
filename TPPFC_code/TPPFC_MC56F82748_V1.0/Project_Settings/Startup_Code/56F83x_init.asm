...
;===============================
        ...
        XREF  F_Livt_size
        XREF  F_Livt_ROM_addr
        XREF  F_Livt_RAM_addr
        org   p:
...
noinc:
        ...
        adda    #2,sp                   ; initialisation of loop counts
        move.l  #-1,r0                  ; end of addressable range
        move.l  r0,x:(sp)
        move.l  x:(sp)-,LA              ; initialised loop count
        adda    #2,sp
        move.l  x:(sp)-,LA2             ; initialised loop count 2
...
; pROM-to-pRAM utility
        ...
        tst.w   r0
        beq     end_prom2pram           ; if no pROM-to-pRAM, then exit
        nop
        move.l  #>>F_Livt_size,r2       ; set data size
        move.l  #>>F_Livt_ROM_addr,r3   ; src address -- pROM data start
        move.l  #>>F_Livt_RAM_addr,r1   ; dest address -- pRAM data start
